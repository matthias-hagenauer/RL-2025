...
def compute_reinforce_loss(policy, episode, discount_factor):
    ...
    for t in reversed(range(len(rewards))):
        ...
        G[t] = running_return
    log_probs = torch.log(policy.get_probs(states).gather(1, actions.long()))
...
def compute_reinforce_loss(policy, episode, discount_factor):
    ...
    for t in reversed(range(len(rewards))):
        running_return = rewards[t] + discount_factor * running_return
        G[t] = running_return
    log_probs = torch.log(policy.get_probs(states, actions).gather(1, actions.long()))
    loss = -torch.sum(log_probs * G)
...
def compute_reinforce_loss(policy, episode, discount_factor):
    ...
    for t in reversed(range(len(rewards))):
        running_return = rewards[t] + discount_factor * running_return
        G[t] = running_return
    log_probs = torch.log(policy.get_probs(states, actions).gather(1, actions.long().view(-1, 1)))
    loss = -torch.sum(log_probs * G)
...
def compute_reinforce_loss(policy, episode, discount_factor):
    ...
    for t in reversed(range(len(rewards))):
        running_return = rewards[t] + discount_factor * running_return
        G[t] = running_return
    log_probs = torch.log(policy.get_probs(states, actions.long().view(-1, 1)).gather(1, actions.long()))
    loss = -torch.sum(log_probs * G)
